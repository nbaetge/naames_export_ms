...
subset <- processed_bf %>% 
  ...
  select(Cruise, Latitude, Longitude, Date, degree_bin, Station, Season, Subregion,  CampCN, 
         Chl_max, Phyto_max, Max_MLD,  Target_Z,  interp_DOC, interp_N_N, interp_NO2, interp_NH4, interp_TDN, interp_PO4, interp_SiO4,  interp_Pro_Influx, interp_Syn_Influx, interp_Pico_Influx, interp_Nano_Influx, Density00, Density11) %>% 
  ...
  ungroup() %>% 
  left_join(., npp) %>% 
  select(Cruise:Phyto_max, Ez, ave_Ez, sd_Ez, Max_MLD,  everything())
```
...
ave_phyto <- subset %>%
  ...
  mutate_at(vars(ave_Ez, sd_Ez, ave_chlmax:sd_pmax), round) %>% 
  ungroup()
...
add.func <- function(morty){
  ...
  schwifty$Target_Z[is.na(schwifty$Target_Z)] <- schwifty$ave_Ez
   meeseek <- schwifty %>% 
     fill(., Cruise:Max_MLD, .direction = c("updown")) %>% 
      fill(., ave_chlmax:sd_pmax, .direction = c("updown")) %>% 
    arrange(CampCN, Target_Z)
...
interpolations.df <- data.frame(Target_Z, interp_doc, interp_don, interp_Si,interp_N, interp_PO4, interp_Pro, interp_Syn,
...
interpolated.df <- right_join(ave_phyto, interpolations.df) %>% 
  select(Cruise:Max_MLD, ave_chlmax:sd_pmax, Target_Z, interp_p, interp_N, interp_PO4, interp_doc, interp_don, interp_Si, interp_Pro:interp_Nano ) %>% 
...
redis_areas <- composite %>% 
  ...
  mutate(redis_N_area = redis_N_vol * Max_MLD,
         ...
         redis_N_ez_area = redis_N_vol * ave_Ez,
         ...
         redis_PO4_area = redis_PO4_vol * Max_MLD,
         redis_PO4_100_area = redis_PO4_vol * 100,
         redis_PO4_ez_area = redis_PO4_vol * ave_Ez,
         redis_PO4_cm_area = redis_PO4_vol * ave_chlmax,
         redis_PO4_phy_area = redis_PO4_vol * ave_pmax,
         redis_DOC_area = redis_DOC_vol * Max_MLD,
         redis_DOC_100_area = redis_DOC_vol * 100,
         redis_DOC_ez_area = redis_DOC_vol * ave_Ez,
         ...
         redis_DON_ez_area = redis_DON_vol * ave_Ez,
...
int_N <- composite %>% 
  ...
  filter(Target_Z <= Max_MLD) %>%
   mutate(int_N_maxmld = integrateTrapezoid(Target_Z, ave_N,type = "A"),
         ...
         int_PO4_maxmld = integrateTrapezoid(Target_Z, ave_PO4,type = "A")) %>% 
  filter(Target_Z <= ave_Ez) %>%
   mutate(int_N_ez = integrateTrapezoid(Target_Z, ave_N,type = "A"),
         int_Si_ez = integrateTrapezoid(Target_Z, ave_Si,type = "A"),
          int_PO4_ez = integrateTrapezoid(Target_Z, ave_PO4,type = "A")) %>% 
  ungroup() %>% 
  ...
  mutate(int_N_100 = integrateTrapezoid(Target_Z, ave_N,type = "A"),
         ...
         int_PO4_100 = integrateTrapezoid(Target_Z, ave_PO4,type = "A"),
         int_PO4_100_vol = integrateTrapezoid(Target_Z, ave_PO4,type = "A")/100) %>% 
...
processed_export <- redis_areas %>% 
  ...
  mutate(delta_N = redis_N_vol - ave_N,
         ...
         delta_PO4 = redis_PO4_vol - ave_PO4,
         ...
         int_delta_PO4_100 = (redis_PO4_100_area - int_PO4_100)/1000,
...
saveRDS(processed_export, "~/GITHUB/naames_export_ms/Output/processed_export_for_bioavMS.6.7.20-2.rds")
saveRDS(processed_export, "~/GITHUB/naames_bioav_ms/Input/processed_export_for_bioavMS.6.7.20-2.rds")
```
...
<img src="ProcessedExport_files/figure-gfm/unnamed-chunk-41-1.png" style="display: block; margin: auto;" />

## Density

<img src="ProcessedExport_files/figure-gfm/unnamed-chunk-43-1.png" style="display: block; margin: auto;" />

<img src="ProcessedExport_files/figure-gfm/unnamed-chunk-45-1.png" style="display: block; margin: auto;" />
