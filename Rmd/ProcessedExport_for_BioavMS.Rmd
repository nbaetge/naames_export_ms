---
title: "Processed Export for Bioavailability MS"
...
subset <- processed_bf %>% 
  ...
  select(Cruise, Latitude, Longitude, Date, degree_bin, Station, Season, Subregion,  CampCN, Max_MLD,  Target_Z,  interp_DOC, interp_N_N, interp_NO2, interp_NH4, interp_TDN, interp_PO4, interp_SiO4, Density00, Density11) %>% 
...
interpolated.df <- right_join(subset, interpolations.df) %>% 
  select(Cruise:Max_MLD, Ez, ave_Ez:sd_Ez, Target_Z, interp_p, interp_N, interp_PO4, interp_Si, interp_doc, interp_don ) %>% 
  group_by(CampCN) %>% 
...
# Calculate ∆NO~3~, NCP, ∆SiO~4~, ∆DOC in Ez
...
saveRDS(processed_export, "~/naames_bioav_ms/Input/processed_export_for_bioavMS.5.29.20.rds")
```
...
n.plot <-  processed_export %>% 
  ...
  ggplot(aes(x = Target_Z, y = interp_N, group = interaction(Station, CampCN))) +
  geom_segment(aes( x = Max_MLD, xend = Max_MLD, y = min(interp_N, na.rm = T), yend = max(interp_N, na.rm = T), group = interaction(Station, CampCN )), colour = "#377EB8", linetype = 3, size = 1.5 ) +
  geom_segment(aes(y = redis_N_vol, yend = redis_N_vol, x = 0, xend = Max_MLD, group = interaction(Station, CampCN ) ), colour = "black", linetype = 2, size = 1.5, alpha = 0.7 ) +
...
doc.plot <-  processed_export %>% 
  filter(Season == "Early Autumn",
         Target_Z %in% c(0, 5, 10, 25, 50, 75, 100, 150, 200, 300, 400, 500, 750)) %>% 
  mutate(bin_plot = paste(degree_bin, "˚N", sep = "")) %>% 
  ggplot(aes(x = Target_Z, y = interp_doc, group = interaction(Station, CampCN))) +
   geom_segment(aes( x = Max_MLD, xend = Max_MLD, y = min(interp_doc, na.rm = T), yend = max(interp_doc, na.rm = T), group = Station ), colour = "#377EB8", linetype = 3, size = 1.5 ) +
